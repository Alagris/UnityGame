...
namespace Env.Runtime
{
    public class ProcMesh
    {
        ...
        {
            return planeRepeatingUVs(offset, resX, resY, size, v => v, genTriangles);
        }
        public static ProcMesh planeRepeatingUVs(float3 offset, int resX, int resY, float2 size, Func<float3, float3> gradFunction, bool genTriangles)
        {
            return plane(offset, resX, resY, size, gradFunction, (int vx, int vy, float3 vertex) => new float2(vx / (resX - 1), vy / (resY - 1)), genTriangles);
        }
        public static ProcMesh planeScaledUVs(float3 offset, int resX, int resY, float2 size, Func<float3, float3> gradFunction, float uvScale, bool genTriangles)
        {
            return plane(offset, resX, resY, size, gradFunction, (int vx, int vy, float3 vertex) => vertex.xy * uvScale, genTriangles);
        }

        /**plane is just a horizontally aligned grid*/

        public static ProcMesh plane(float3 offset, int resX, int resY, float2 size, Func<float3, float3> gradFunction, Func<int, int, float3, float2> uvTransform, bool genTriangles)
        {
            return grid(offset, resX, resY, new float3(size.x, 0, 0), new float3(0, 0, size.y), new float3(0, 1, 0), gradFunction, uvTransform, genTriangles);
        }
        ...
        {
            return forEachPosition2D(offset, resX, resZ, 0, widthX, heightZ, f);

        }
        public static T[] forEachPosition2D<T>(float2 offset, int resX, int resZ, int resPadding, float widthX, float heightZ, Func<int, float2, T> f)
        {

            float spacingX = widthX / (resX - 1);
            float spacingZ = heightZ / (resZ - 1);
            offset -= new float2(spacingX * resPadding, spacingZ * resPadding);
            resX += resPadding * 2;
            resZ += resPadding * 2;
            T[] o = new T[resZ * resX];
            for (int vz = 0, i = 0; vz < resZ; vz++)
        ...
        public static ProcMesh gridPrecomputed(float3 offset, int resX, int resZ, float widthX, float heightZ, float[] heights, float uvScaling)
        {
            ...
            for (int vz = 0, i = 0; vz < resZ; vz++)
            {
                for (int vx = 0; vx < resX; vx++, i++)
                {
                    int left = (vz + 1) * resXPadded + vx;
                    int center = (vz + 1) * resXPadded + vx + 1;
                    int right = (vz + 1) * resXPadded + vx + 2;
                    int up = (vz + 2) * resXPadded + vx + 1;
                    int down = vz * resXPadded + vx + 1;
                    float centerH = heights[center];
                    float leftH = heights[left];
                    float rightH = heights[right];
                    float upH = heights[up];
                    ...
                    //   (rightH-centerH)/spacingX
                    // average of the two:
                    //   ((rightH-centerH)/spacingX+(centerH-leftH)/spacingX)/2 =
                    //   (rightH-centerH+centerH-leftH)/spacingX/2 =
                    //   (rightH-leftH)/(2*spacingX)
                    float derX = (rightH - leftH) / (2 * spacingX);
                    float derZ = (upH - downH) / (2 * spacingZ);
                    float3 normal = -math.normalize(BMath.normal(new float2(derX, derZ)));
                    normals[i] = normal;
                }
            }
            o.vertices = vertices;
        ...
        {
            return planeScaledUVs(offset, resX, resY, size, vertex => Noise.morenoise_derivative(vertex.xz, scale, pointiness, scalingPowerBase, iterations) * height, uvScale, genTriangles);
        }
