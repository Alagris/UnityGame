...
namespace Env.Runtime
{
    public class Noise
    {
        ...
        public static float3 morenoise_derivative(float2 position, float scale, float pointiness, float scalePowerBase, int iterations)
        {
            float3 der_and_height = morenoise_derivative(position / scale, pointiness, scalePowerBase, iterations);
            der_and_height.x /= scale;
            der_and_height.y /= scale;
            return der_and_height;
        }
        public static float3 morenoise_derivative(float2 position, float pointiness, float scalePowerBase, int iterations)
        {
        ...
        public static float morenoise(float2 position, float scale, float pointiness, float scalePowerBase, int iterations)
        {
            return morenoise(position / scale, pointiness, scalePowerBase, iterations);
        }
        public static float morenoise(float2 position, float pointiness, float scalePowerBase, int iterations)
        {
            float2 c = new float2(0f, 0f);//cummulative first order derivative of perlin noise
